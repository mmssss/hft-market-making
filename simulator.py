...
class Order:
    ...
    Non-optional attributes must be not None, otherwise ValueError is
    raised.

    Args:
        client_ts:
            Timestamp (in nanoseconds) of when the order was created
            in the client time. It can be equal to `receive_ts` of the
            last update from the strategy returned by method `tick`, or
            it can be slightly greater, if you want to account for code
            execution time.
        side:
        ...
        price:
            Order price.
        exchange_ts:
            Timestamp (in nanoseconds) of when the order was registered
            on the exchange simulator. Filled in by the simulator.
        receive_ts:
            Timestamp (in nanoseconds) of when the data is received by
            client. Exchange simulator fills in this attribute,
            emulating latency.
        order_id:
            Unique ID assigned by the exchange simulator when the order
            is placed. Filled in by the simulator.
        client_order_id:
            Unique ID assigned by client. Can be used to immediately
...
class OrderCancel:
    ...
    Non-optional attributes must not be `None`, otherwise `ValueError`
    is raised. Either `client_order_id` or `order_id` must be provided.
    If both are provided, `order_id` takes precedence.

    Attributes:
        client_ts:
            Timestamp (in nanoseconds) of when the cancel was created
            in the client time.
        exchange_ts:
            Timestamp (in nanoseconds) of when the cancel was registered
            on the exchange simulator. Filled in by the simulator.
        receive_ts:
            Timestamp (in nanoseconds) of when the data is received
            by client. Simulator fills in this attribute, emulating
            latency.
        order_id:
            Unique ID that was assigned by the exchange simulator when
    ...
    def __post_init__(self) -> None:
        ...
        if self.client_order_id is None and self.order_id is None:
            raise ValueError('Either `client_order_id` or `order_id` must be'
                             'provided')
...
class ResponseCode(IntEnum):
    ...
    Attributes:
        ...
        PLACE_ORDER_INSUFFICIENT_BALANCE:
            Failed to create the order, because account has insufficient
            balance.
...
class OwnTrade:
    ...
    An object is created by exchange simulator when client's order is
    filled.

    Attributes:
        exchange_ts:
            Timestamp (in nanoseconds) of when the trade occured on the
            exchange simulator.
        receive_ts:
            Timestamp (in nanoseconds) of when the data is received by
            client. Exchange simulator fills in this attribute,
        ...
        price:
            Trade price.
        client_order_id:
            Client-assigned unique ID of the order that has been
            executed.
...
class OrderbookSnapshotUpdate:
    ...
    Attributes:
        exchange_ts:
            Timestamp (in nanoseconds) in the exchange time (as recorded
            during the real-world data collection).
        receive_ts:
            Timestamp (in nanoseconds) of when the data was received by
            client (as recorded during the real-world data collection).
        asks:
...
def load_md_from_files(lobs_path: str, trades_path: str,
    ...
    then creates a queue of `MdUpdate` objects, ordered by data
    ...
    Args:
        ...
        min_ts:
            If provided, market data updates with reception timestamps
            less than `min_ts` will not be included in the
            resulting queue.
        max_ts:
            If provided, market data updates with reception timestamps
            greater than `max_ts` will not be included in the
            resulting queue.
        ...
        while i < len(lobs_ts) and j < len(trades_ts):
            ...
            if lobs_ts[i][0] <= trades_ts[j][0]:
                ...
                orderbook = OrderbookSnapshotUpdate(
                    exchange_ts=lobs_ts[i][1], receive_ts=lobs_ts[i][0],
                    asks=asks, bids=bids)
                i += 1
            # compare receive_ts
            elif trades_ts[j][0] <= lobs_ts[i][0]:
                trade = AnonTrade(exchange_ts=trades_ts[j][1],
                                  receive_ts=trades_ts[j][0], side=trades[j][0],
                                  size=trades[j][2], price=trades[j][1])
                j += 1
...
class ExchangeSimulator:
    ...
    1. initialize the object of this class, providing parameters and
       path to market data;
    2. call method `tick` to get an update from the simulator.
       The update will contain two timestamps:
    ...
    3. call methods `place_order` and `cancel_order` according to your
       strategy, using `receive_ts` from the previous step as the
       current client time to calculate the value of parameter
       `client_ts` (see `place_order` for details).
    """
    ...
    def __init__(self, lobs_path: str, trades_path: str,
        ...
        Args:
            ...
            trades_path:
                Path to csv file which contains trades data.
            exec_latency:
                Latency in nanoseconds between the moment when the
                client executes an action and the moment when this
                action is registered on the exchange.
            updates_latency:
                Latency in nanoseconds between the moment when the
                action is registered in the simulator, and the moment
                when the client receives an update from the simulator.
            account_size:
        ...
        # Every tick it is set to the smallest event time of three
        # queues: market data, strategy actions, strategy updates.
        ...
        # Market data deque. Elements should be popped from the left.
        self.md: Deque[MdUpdate] = \
            load_md_from_files(lobs_path, trades_path, min_ts, max_ts)
        # The dequeue of strategy actions, used to simulate execution
        # latency. Elements should be appended to the right, and popped
        # from the left.
        self.actions: Deque[Union[Order, OrderCancel]] = deque()
        # Priority queue (using heapq) for the data that is sent to
        # the strategy. Each element in queue is a tuple
        # `(update_obj.receive_ts, counter, update_obj)`,
        # where 'counter' is used to break comparison ties.
        self.strategy_updates: \
            List[Tuple[int, int, Union[MdUpdate, OwnTrade, ActionResponse]]] = []
        # Counter for items added into `self.strategy_updates` queue.
        self.strategy_updates_counter: int = 0
        # For now only store best bid and ask levels on the simulator
        # side and assume the client orders are small enough to always
        # be fully executed.
        ...
        # Key: client_order_id, Value: `True` if this ID has already
        # been used, `False` otherwise.
        self.used_client_ids: Dict[int, bool] = dict()
        # Dictionary for all active client orders.
        # Key: tuple(0, order_id) or tuple(1, client_order_id).
        # Value: `Order` object.
        # Such key structure allow to, for example, cancel the order
        # using either its order_id or client_order_id.
        ...
        #     Key: tuple(0, order_id) or tuple(1, client_order_id).
        #     Value: `Order` object.
        self.active_orders_ask: \
            SortedDict[int, OrderedDict[(int, int), Order]] = SortedDict()
        self.active_orders_bid: \
            SortedDict[int, OrderedDict[(int, int), Order]] = SortedDict()
    ...
    def tick(self) -> Union[MdUpdate, OwnTrade, ActionResponse, None]:
        ...
        Returns:
            ...
            `ActionResponse`:
                Response to client's action (placing or canceling an
                order). Contains the copy of the action object that was
                passed to the simulator, with extra filled attributes.
                Also contains execution status and possibly
        ...
        while True:
            ...
            if md_et == math.inf and actions_et == math.inf \
                    and updates_et == math.inf:
                return None
                ...
                if type(action) == Order:
                    ...
                    if order.side == 'BID':
                        if quote_size <= self.account_size:
                            ...
                            self._push_action_response(
                                order, ResponseCode.PLACE_ORDER_INSUFFICIENT_BALANCE)
                            continue
                    else:
                    ...
                    if order.side == 'BID':
                        orders = self.active_orders_bid.setdefault(
                            order.price, OrderedDict())
                    else:
                        orders = self.active_orders_ask.setdefault(
                            order.price, OrderedDict())
                    # use `order_id` as key
                ...
                # Cancel order action has waited the execution latency
                    ...
                    if cancel.order_id is not None:
                        ...
                        if key1 in self.active_orders:
                            ...
                            self._push_action_response(
                                cancel, ResponseCode.CANCEL_ORDER_ID_NOT_FOUND)
                            continue
                        ...
                        if key1 in self.active_orders:
                            ...
                            self._push_action_response(
                                cancel, ResponseCode.CANCEL_ORDER_CLIENT_ID_NOT_FOUND)
                            continue
                    else:
                        raise RuntimeError(
                            'Either `client_order_id` or `order_id` must be'
                            'provided in `OrderCancel` object')
                    ...
                    if order.side == 'BID':
                        ...
                        # TODO: (2) maybe freeze a part of the position
    ...
    def _get_md_event_time(self) -> Union[int, float]:
    ...
    def _get_actions_event_time(self) -> Union[int, float]:
    ...
    def _get_strat_updates_event_time(self) -> Union[int, float]:
    ...
    def _push_strategy_update(
            self, receive_ts,
            update: Union[MdUpdate, OwnTrade, ActionResponse]) -> None:
        self.strategy_updates_counter += 1
        heapq.heappush(self.strategy_updates,
                       (receive_ts, self.strategy_updates_counter, update))

    def _push_action_response(self, action: Union[Order, OrderCancel],
        ...
        for price in self.active_orders_ask:
            if price < self.best_bid:
                while self.active_orders_ask[price]:
                    self._execute_orders_on_price_level(
                        self.active_orders_ask, price)
            else:
                break
        for price in reversed(self.active_orders_bid):
            if price > self.best_ask:
                while self.active_orders_bid[price]:
                    self._execute_orders_on_price_level(
                        self.active_orders_bid, price)
            else:
                break

    def _execute_orders_on_price_level(self, active_orders_bidask, price) -> None:
        # We know that the key is `order_id`, because we first add the
    ...
    def place_order(self, order: Order) -> None:
        """Places the order in simulator, with latency emulation.

        A copy of passed `Order` object is created.

        Args:
            order:
                `Order` object. See `Order` for information about the
                attributes. The same `client_order_id` can only be used
                once per simulation.
        ...
        Raises:
            ...
            RuntimeError:
                If tried to place an order with client order ID that has
                already been used. This situation does not depend on
                emulated latency and is caused by a client failing to
                generate unique IDs for the orders. Thus, it makes
                more sense to raise Python error instead of returning
        ...
        if order.client_order_id in self.used_client_ids:
            raise RuntimeError(f'Client order id {order.client_order_id}'
                               f'has already been used')
        order_copy = copy.deepcopy(order)
        order_copy.exchange_ts = order_copy.client_ts + self.exec_latency
        self.actions.append(order_copy)

    def cancel_order(self, order_cancel: OrderCancel) -> None:
        """Cancels the order in simulator, with latency emulation.

        A copy of passed `OrderCancel` object is created.

        Args:
            order_cancel:
                `OrderCancel` object. See `OrderCancel` for information
